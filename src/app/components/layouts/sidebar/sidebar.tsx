...
const Sidebar = ({ local_varaiable, ThemeChanger }: any) => {
	...
	// function mapModuleToMenuItem(mod: ModuleTree): any {
	// 	const icon        = MODULE_ICONS[mod.icon ?? 'default'] ?? MODULE_ICONS['default'];
	// 	const hasChildren = mod.children.length > 0;
	// 	if (hasChildren) {
	// 		return {
	// 			title: mod.name, icon, type: 'sub',
	// 			active: false, selected: false,
	// 			children: mod.children.map(mapModuleToMenuItem),
	// 		};
	// 	}
	// 	return {
	// 		title: mod.name, icon,
	// 		path: mod.url_path ?? '#',
	// 		type: 'link', active: false, selected: false, children: [],
	// 	};
	// }

	// Agrego useEffect que escucha el árbol de Redux
	// useEffect(() => {
	// 	if (moduleTree.length > 0) {
	// 		setMenuitems(moduleTree.map(mapModuleToMenuItem));
	// 	}
	// }, [moduleTree]);
