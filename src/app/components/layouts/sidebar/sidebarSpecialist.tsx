...
import { MENUITEMSSPECIALIST } from './nav';

const SidebarSpecialist = ({ local_varaiable, ThemeChanger }: any) => {
	const { user } = useAuth();
	...
	function closeMenu() {
		...
		closeMenudata(MENUITEMSSPECIALIST);
	...
	function setSubmenu(event: any, targetObject: any, MenuItems = MENUITEMSSPECIALIST) {
		const theme = store.getState().theme;
	...
	function setMenuAncestorsActive(targetObject: any) {
		const parent = getParentObject(MENUITEMSSPECIALIST, targetObject);
		const theme = store.getState().theme;
		if (parent) {
			if (hasParentLevel > 2) {
				hasParent = true;
	...
	function setMenuUsingUrl(currentPath: any) {
		...
		setSubmenuRecursively(MENUITEMSSPECIALIST);
	}
	...
	function toggleSidemenu(event: any, targetObject: any, MenuItems = MENUITEMSSPECIALIST, state?:any) {
		const theme = store.getState().theme;
				...
				<SimpleBar className='main-sidebar' id="sidebar-scroll">
					<nav className='main-menu-container nav nav-pills flex-column sub-open'>
						...
						<ul className="main-menu" onClick={() => Sideclick()}>
							{MENUITEMSSPECIALIST.map((levelone: any, index: any) => (
								<Fragment key={index}>
...
export default connect(mapStateToProps, { ThemeChanger })(SidebarSpecialist);
