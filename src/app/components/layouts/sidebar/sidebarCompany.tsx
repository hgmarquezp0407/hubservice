...
// import { MENUITEMSCOMPANY } from './nav';
import { useSelector } from 'react-redux';
import { MODULE_ICONS } from '@/utils/moduleIcons';
// import type { ModuleTree } from '@/redux/slices/modulesSlice';
...
const SidebarCompany = ({ local_varaiable, ThemeChanger }: any) => {
	const { user } = useAuth();

	// const moduleTree    = useSelector((state: RootState) => state.modules.tree);
	...
	function closeMenu() {
		...
		closeMenudata(MENUITEMSCOMPANY);
	...
	function setSubmenu(event: any, targetObject: any, MenuItems = MENUITEMSCOMPANY) {
		const theme = store.getState().theme;
	...
	function setMenuAncestorsActive(targetObject: any) {
		const parent = getParentObject(MENUITEMSCOMPANY, targetObject);
		const theme = store.getState().theme;
		if (parent) {
			if (hasParentLevel > 2) {
				hasParent = true;
	...
	function setMenuUsingUrl(currentPath: any) {
		...
		setSubmenuRecursively(MENUITEMSCOMPANY);
	}
	...
	function toggleSidemenu(event: any, targetObject: any, MenuItems = MENUITEMSCOMPANY, state?:any) {
		const theme = store.getState().theme;
		...
		for (const mutation of mutationsList) {
			if (mutation.type === 'attributes' && mutation.attributeName === 'data-nav-layout') {
				...
				if (newValue === 'vertical') {
					// Obtener el path actual sin modificar basePath
					let currentPath = location.pathname.endsWith('/') ? location.pathname.slice(0, -1) : location.pathname;
					
					// Si el path está vacío, usar path por defecto
					currentPath = currentPath || '/admin/home';
				...
				<SimpleBar className='main-sidebar' id="sidebar-scroll">
					<nav className='main-menu-container nav nav-pills flex-column sub-open'>
						...
						<ul className="main-menu" onClick={() => Sideclick()}>
							{MENUITEMSCOMPANY.map((levelone: any, index: any) => (
								<Fragment key={index}>
...
export default connect(mapStateToProps, { ThemeChanger })(SidebarCompany);