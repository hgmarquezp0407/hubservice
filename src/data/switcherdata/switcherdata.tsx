...
export function Dark(actionfunction: any, clicked?: any) {
...
export function Light(actionfunction: any, clicked?: any) {
...
function closeMenu() {
    document.querySelectorAll<HTMLElement>(".slide.open").forEach(el => {
        el.classList.remove("open");
    });
    document.querySelectorAll<HTMLElement>(".slide-menu").forEach(el => {
...
export const iconOverayFn = (actionfunction: any) => {
    ...
    // const icon: any = document.getElementById("switcher-icon-overlay");
    // if (icon) {
    // 	icon.checked = true;
    // }
    const MainContent = document.querySelector(".main-content");
...
export const LocalStorageBackup = (actionfunction: any) => {
    ...
    if (localStorage.xintraverticalstyles) {
        const verticalStyles = localStorage.getItem("xintraverticalstyles");
    ...
    if (localStorage.bodyBg) {
        const updateddarkBg = `${Number(localStorage.bodyBg.split("")[0]) + 14} ${Number(localStorage.bodyBg.split("")[1]) + 14} ${Number(localStorage.bodyBg.split("")[2]) + 14}`
        const theme = store.getState()
        actionfunction({
            ...theme,
